...
namespace Toggl.Core.Tests.UI.ViewModels
{
    public sealed class StartTimeEntryViewModelTests
    {
        ...
        public sealed class TheInitializeMethod : StartTimeEntryViewModelTest
        {
            ...
            [Fact, LogIfTooSlow]
            public async Task StarsTheTimerWhenDurationIsNull()
            {
            ...
            [Fact, LogIfTooSlow]
            public async Task SetsTheDisplayedTimeToTheValueOfTheDurationParameter()
            {
        ...
        public abstract class CreateSuggestionCellModels : StartTimeEntryViewModelTest
        {
            ...
            public async Task WontExistIfSuggestingTimeEntries()
            {
                ...
                await ViewModel.OnTextFieldInfoFromView(new QueryTextSpan("", 0)).FirstAsync();
            ...
            public async Task WontExistIfTheCurrentQueryIsEmpty()
            {
                ...
                await ViewModel.OnTextFieldInfoFromView(new QueryTextSpan($"{QuerySymbol}", 1)).FirstAsync();
            ...
            public async Task WontExistIfTheCurrentQueryIsLongerThanMaxLength()
            {
                ...
                await ViewModel.OnTextFieldInfoFromView(new QueryTextSpan($"{QuerySymbol}{createLongString(MaxLength + 1)}", 1)).FirstAsync();
            ...
            public async Task WontExistIfSuchSuggestionAlreadyExists()
            {
                ...
                await ViewModel.OnTextFieldInfoFromView(new QueryTextSpan($"{QuerySymbol}{QueryWithExactSuggestionMatch}", 1)).FirstAsync();
            ...
            public sealed class WhenSuggestingProjects : CreateSuggestionCellModels
            {
                ...
                public async Task WontExistIfAProjectIsAlreadySelected()
                {
                    ...
                    await ViewModel.OnTextFieldInfoFromView(projectSpan).FirstAsync();
                    await ViewModel.OnTextFieldInfoFromView(projectSpan, querySpan).FirstAsync();
                ...
                public async Task ExistIfTheProjectNameIsValid()
                {
                    ...
                    await ViewModel.OnTextFieldInfoFromView(new QueryTextSpan($"@{projectName}", 6)).FirstAsync();
                ...
                public async Task TracksProjectSelection()
                {
                    ...
                    await ViewModel.OnTextFieldInfoFromView(new QueryTextSpan("abcde @fgh", 10)).FirstAsync();
        ...
        public sealed class TheToggleProjectSuggestionsAction : StartTimeEntryViewModelTest
        {
            ...
            public async Task ShowsCorrectProjectSuggestionsAfterProjectHasAlreadyBeenSelected()
            {
                ...
                await ViewModel.Initialize(DefaultParameter);
                await ViewModel.OnTextFieldInfoFromView(new QueryTextSpan(Description, Description.Length)).FirstAsync();
        ...
        public sealed class TheToggleTagSuggestionsCommand : StartTimeEntryViewModelTest
        {
            ...
            [Fact, LogIfTooSlow]
            public async Task SetsTheIsSuggestingTagsPropertyToTrueIfNotInTagSuggestionMode()
            {
                await ViewModel.Initialize(DefaultParameter);
                ViewModel.Suggestions.Subscribe();
                var observer = TestScheduler.CreateObserver<bool>();
        ...
        public sealed class TheSetStartDateCommand : StartTimeEntryViewModelTest
        {
            ...
            {
                await ViewModel.Initialize(prepareParameters);
            ...
            public async Task OpensTheSelectDateTimeViewModelWithCorrectLimitsForAStoppedTimeEntry()
            {
                ...
                await ViewModel.Initialize(stoppedParametsrs);
        ...
        public sealed class TheTextFieldInfoProperty : StartTimeEntryViewModelTest
        {
            ...
            public async Task DoesNotChangeSuggestionsWhenOnlyTheCursorMovesForward(string text)
            {
                ...
                await ViewModel.Initialize(DefaultParameter);
                await ViewModel.OnTextFieldInfoFromView(new QueryTextSpan(text, text.Length)).FirstAsync();

                await ViewModel.OnTextFieldInfoFromView(new QueryTextSpan(text, 0)).FirstAsync();
            ...
            public async Task ChangesSuggestionsWhenTheCursorMovesBackBehindTheOldCursorPosition(string text)
            {
                ...
                await ViewModel.Initialize(DefaultParameter);
                await ViewModel.OnTextFieldInfoFromView(new QueryTextSpan(extendedText, text.Length)).FirstAsync();
                await ViewModel.OnTextFieldInfoFromView(new QueryTextSpan(extendedText, 0)).FirstAsync();

                await ViewModel.OnTextFieldInfoFromView(new QueryTextSpan(extendedText, extendedText.Length)).FirstAsync();
                TestScheduler.Start();
